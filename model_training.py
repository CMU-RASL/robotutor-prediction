...
def fit_model(params):
    train_X, train_Y, train_T, start_val, end_val, filename, num_classes, \
            foldername, model_type, max_depth, tot_model_num = params
    ...
    if len(X) > 0:
        ...
        for ii in range(num_classes):
            if not ii in unique_elements:
                ...
                class_weight[ii] = counts_elements[np.where(unique_elements == ii)[0][0]]/np.sum(counts_elements)

        model = RandomForestClassifier(max_depth=max_depth, class_weight=class_weight)
        model.fit(flat_train_X, flat_train_Y)
        dump(model, 'models//' + foldername + '//' + filename + '.joblib')
...
def create_all_models(foldername, num_models, class_num_arr,
    num_workers, X, Ys, T, train_inds, model_bool, max_depth):
    ...
    for model_type in range(len(model_split)):
        ...
        tot_model_num[model_type] = tot_model_num[model_type]
        ...
        for model_type in range(len(model_split)):
            for fold_ind in range(len(train_inds)):
                for model_start_ind in range(model_split[model_type].shape[1]-1):
                    ...
                    model_name = 'Modeltype_{}_Start_{}_End_{}_Fold_{}'.format(
...
def get_acc(model_foldername, modeltype_ind, model_split, fold_ind, k, Xk,
        Yk, Tk, thresh_arr, incr_arr, class_weight, num_classes, plot_foldername, plot_bool):
    cur_model_arr = [[],[]]
    for model_start_ind in range(model_split.shape[0]-1):
        ...
        cur_models = []
        model_name = 'Modeltype_{}_Start_{}_End_{}_Fold_{}.joblib'.format(
                modeltype_ind, start_val, end_val, fold_ind)
    ...
    acc, early = run_models(Xk, Yk, Tk, cur_model_arr,
                    thresh_arr, incr_arr, class_weight, num_classes =
                    num_classes,
